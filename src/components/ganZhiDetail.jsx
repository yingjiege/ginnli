const tianGanWuXing = [
  ...
  function timeDetermine(time) {
    const [timeHour, timeMinute] = time.split(':').map(Number);
    const totalMinutes = timeHour * 100 + timeMinute;  // Convert HH:MM to an integer like HHMM
    let timeGanzhi = '';
  
    if (totalMinutes >= 2300 || totalMinutes <= 59) {
      timeGanzhi = '子';
    } else if (totalMinutes >= 100 && totalMinutes <= 259) {
      timeGanzhi = '丑';
    } else if (totalMinutes >= 300 && totalMinutes <= 459) {
      timeGanzhi = '寅';
    } else if (totalMinutes >= 500 && totalMinutes <= 659) {
      timeGanzhi = '卯';
    } else if (totalMinutes >= 700 && totalMinutes <= 859) {
      timeGanzhi = '辰';
    } else if (totalMinutes >= 900 && totalMinutes <= 1059) {
      timeGanzhi = '巳';
    } else if (totalMinutes >= 1100 && totalMinutes <= 1259) {
      timeGanzhi = '午';
    } else if (totalMinutes >= 1300 && totalMinutes <= 1459) {
      timeGanzhi = '未';
    } else if (totalMinutes >= 1500 && totalMinutes <= 1659) {
      timeGanzhi = '申';
    } else if (totalMinutes >= 1700 && totalMinutes <= 1859) {
      timeGanzhi = '酉';
    } else if (totalMinutes >= 1900 && totalMinutes <= 2059) {
      timeGanzhi = '戌';
    } else if (totalMinutes >= 2100 && totalMinutes <= 2259) {
      timeGanzhi = '亥';
    }
  ...
  // 计算八字函数
  export function getGanZhiDetail(year, month, day, time) {
    // 计算年柱天干和地支
    ...
    const TimeGanZhi = timeDetermine(time)

    // 组合成八字
    const bazi = `${yearTianGan}${yearDiZhi} ${monthTianGan}${monthDiZhi} 
    ${dayTianGan}${dayDiZhi} ${TimeGanZhi}`;
  
    console.log(TimeGanZhi)
    console.log(bazi)
    // 计算五行数量
